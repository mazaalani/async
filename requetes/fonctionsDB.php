<?php
	...
	function getAllEquipes() {
		return executeRequete("SELECT id, nom, quartier FROM equipe");		
	}

	function addTeam($nom, $quartier) {
		return executeRequete("INSERT INTO equipe (nom, quartier) 
							   VALUES ('$nom', '$quartier')", true); 		// On veut le dernier id inseré
	}

	function changeTeamName($nom, $id) {
		return executeRequete("UPDATE equipe 
							   SET nom = '" . $nom . "' 
							   WHERE id =" . $id);
	}

	function deleteTeam($id) {
		return executeRequete("DELETE FROM equipe WHERE id=" . $id);
	}
?>